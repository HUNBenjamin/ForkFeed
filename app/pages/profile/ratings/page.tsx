...
export default function MyRatingsPage() {
  ...
  const fetchRecipes = async (p: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/users/${user.id}/ratings?page=${p}&limit=12`);
      if (!res.ok) throw new Error("Nem sikerült betölteni az értékeléseket.");
      ...
      <div className="max-w-5xl mx-auto px-5 py-8 flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold">Értékelt receptek</h1>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {recipes.map((r) => (
                <RatedRecipeCard key={r.id} recipe={r} />
              ))}
            </div>
