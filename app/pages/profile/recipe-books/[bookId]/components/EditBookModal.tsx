...
type Props = {
  book: { name: string; description: string | null; is_public: boolean };
  open: boolean;
  onClose: () => void;
  onSaved: () => void;
  bookId: number;
...
export default function EditBookModal({ book, open, onClose, onSaved, bookId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error ?? "Nem sikerült menteni a módosításokat.");
      }
  ...
  return (
    <div className="modal modal-open">
      <div className="modal-box">
        <h3 className="font-bold text-lg mb-4">Receptfüzet szerkesztése</h3>
